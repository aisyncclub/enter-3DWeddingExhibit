...
import { Button, SHADOW_CARD, SHADOW_SECONDARY } from './Button';
import { useInViewAnimation } from './useInViewAnimation';
import { PHOTOS } from './data';
import {
  CalendarGlyph, ClockGlyph, PinGlyph, CompassGlyph, EnvelopeGlyph,
  PenGlyph, PersonGlyph, PhoneGlyph, CopyGlyph, CheckGlyph,
  ChevronDownGlyph, BloomGlyph,
} from '../components/icons/EditorialIcons';

/* ── v2 tone tokens (white / ink) ─────────────────────────────── */
export const INK = '#051A24';
export const INK2 = '#0D212C';
export const MUTED = '#273C46';
export const SUBTLE = 'rgba(5,26,36,0.45)';
export const LINE = 'rgba(5,26,36,0.10)';
...
const STORY = [
  { num: '01', title: '처음 만난 날', sub: '2022년 가을 전시회에서', text: '두 사람은 같은 사진 앞에서 오래 머물다 눈이 마주쳤습니다. 어색한 미소와 함께 건넨 첫 마디가, 두 시간의 이야기로 이어졌습니다.', idx: 1 },
  { num: '02', title: '함께 걷기 시작한 계절', sub: '2023년 봄, 비 오던 오후', text: '우산 하나를 나눠 쓰며 비 오는 골목을 걸었습니다. 발이 젖는 줄도 모르고 웃으며 걷다 보니, 어느새 집 앞에 다다랐습니다.', idx: 2 },
  { num: '03', title: '서로의 가족이 된 순간', sub: '2024년 여름, 부모님의 생신날', text: '서로의 생일보다 상대방 부모님의 안부를 먼저 챙기게 되었을 때, 두 사람은 이미 가족이 되어 있었습니다.', idx: 3 },
  { num: '04', title: '결혼을 약속한 밤', sub: '2025년 겨울, 별이 맑던 밤', text: '밤하늘 아래 작은 편지 한 장과 반지 하나. 말 대신 눈물이 먼저였습니다. 그날의 별빛이, 이제 두 사람의 약속이 되었습니다.', idx: 4 },
];

const GALLERY_IDX = [0, 5, 2, 6, 3, 7];
...
export const Serif = ({ children, italic = false }: { children: ReactNode; italic?: boolean }) => (
...
/* ════════════════════════════════════════════════════════════════
   InvitationView — the full invitation body, fully driven by `data`.
   Used by the /demo sample and by the /create live preview.
   ════════════════════════════════════════════════════════════════ */
export function InvitationView({ data }: { data: InvitationData }) {
  ...
  // photo accessor that cycles, so any number of images fills every slot
  ...
  const submitGb = (e: React.FormEvent) => {
    ...
    setMsgs((prev) => [{ name: gbName, msg: gbMsg, date: '오늘' }, ...prev]);
    setGbName(''); setGbMsg('');
  };
  ...
  return (
    <div className="relative z-10 max-w-[480px] mx-auto px-6">
      ...
      <Reveal delay={0.1} className="overflow-hidden rounded-[28px] mt-6" style={{ boxShadow: '0 18px 50px rgba(5,26,36,0.16)' }}>
        <img src={p(0)} alt="" crossOrigin="anonymous" className="w-full h-[440px] object-cover" />
      </Reveal>
      ...
      <section className="pt-16">
        ...
        <Reveal delay={0.4}><p className="text-center v2-serif text-sm mt-8" style={{ color: SUBTLE }}>박{data.groomName} · 이{data.brideName} 올림</p></Reveal>
        <Ornament />
      </section>
      ...
      <section>
        ...
        <Reveal className="rounded-[28px] overflow-hidden" style={{ background: SURF, boxShadow: SHADOW_CARD }}>
          {[
            { Icon: CalendarGlyph, label: 'Date', value: data.fullDate },
            { Icon: ClockGlyph, label: 'Time', value: data.time },
            { Icon: PinGlyph, label: 'Hall', value: data.hall },
            { Icon: PinGlyph, label: 'Addr', value: data.addr },
      ...
      <section>
        ...
        <Reveal className="rounded-[28px] flex flex-col items-center justify-center py-12" style={{ background: SURF }}>
          ...
          <p className="font-medium text-sm mt-3" style={{ color: INK }}>{data.hall}</p>
          <p className="text-xs mt-1" style={{ color: MUTED }}>{data.addr}</p>
        </Reveal>
        ...
        <Reveal delay={0.2} className="mt-4">
          <button onClick={() => copy(data.hall, 'map')}
      ...
      <section className="text-center pb-10">
        <Reveal className="overflow-hidden rounded-[28px] mb-10" style={{ boxShadow: SHADOW_CARD }}>
          <img src={p(7)} alt="" crossOrigin="anonymous" className="w-full h-72 object-cover" />
        </Reveal>
        <Reveal>
          <Label en="Thank You" />
          ...
          <p className="v2-serif text-xl mt-7" style={{ color: INK }}>{data.groomName} &amp; {data.brideName}</p>
        </Reveal>
